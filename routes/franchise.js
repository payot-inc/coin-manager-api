...
router.get('/:id', [
    ...
    franchise.findOne({
        ...
        include: [
            { model: company, attributes: { exclude: ['hash', 'salt', 'franchiseId', 'createdAt', 'updatedAt', 'deletedAt'] }}
        ],
...
router.post('/', [
    ...
    check('tel', '전화번호를 입력해 주세요').isString().isLength({ min: 9, max: 20 }).optional()
], (req, res) => {
    const errors = validationResult(req);
...
router.put('/:id', [
    ...
    check('tel', '주소를 입력해 주세요').isString().optional()
], (req, res) => {
    const errors = validationResult(req);
